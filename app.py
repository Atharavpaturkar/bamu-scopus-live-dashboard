...
sb_logo_html = f'<img src="{bamu_sb_b64}" alt="BAMU Logo" style="height: 72px; width: auto; max-width: 78px; object-fit: contain; background: transparent !important; background-color: transparent !important; border: none; margin: 0 auto 0.6rem auto; display: block; filter: {sb_logo_filter};">' if bamu_sb_b64 else ''
...
with tab5:
    ...
    if not leaderboard_df.empty:
        ...
        if auth_profile:
            # Header Card (Matching Primary Blue Button Background Aesthetic)
            st.markdown(f"""
            <div class="univ-header-card" style="margin-bottom: 1.5rem; padding: 1.1rem 1.4rem;">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                    <div>
                        <h2 style="margin: 0; font-family: 'Outfit', sans-serif; font-size: 1.65rem; font-weight: 800; color: #FFFFFF !important;">{auth_profile['author_name']}</h2>
                        <div style="font-size: 0.9rem; font-weight: 600; color: #E0F2FE !important; margin-top: 0.3rem;">
                            {auth_profile['primary_department']} • Institutional Faculty Scopus Dossier
                        </div>
                    </div>
                    <div style="background: rgba(255, 255, 255, 0.18); border: 1px solid rgba(255, 255, 255, 0.35); border-radius: 8px; padding: 0.4rem 0.8rem; font-family: 'Outfit', sans-serif; font-size: 0.82rem; font-weight: 700; color: #FFFFFF;">
